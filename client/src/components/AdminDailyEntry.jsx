...
export default function AdminDailyEntry(props) {
        ...
        .map((user) => {
          const todayEntry = entryListRaw.find(
            (entry) => {
              ...
              const entryDateFormatted = (entry.date instanceof Date) 
          ...
          return {
            ...
            date: (todayEntry?.date && typeof todayEntry.date === 'string' && todayEntry.date.includes('T'))
  ...
  async function handleFormSubmit(entry) {
    ...
    } else if (typeof entryForBackEnd.date === 'string' && entryForBackEnd.date.includes('T')) {
      ...
      try {
        ...
        setEntryList(prevList => {
            const newList = [...prevList, {
                ...
                updateDate: addedEntryFromServer.updateDate || new Date().toISOString(),
            }];
            return newList.sort((a, b) => new Date(b.updateDate || 0) - new Date(a.updateDate || 0));
        });
  ...
  async function handleModalQtySubmit() {
    ...
    const currentSelectedDate = resolveSelectedDate(selectedDateOption, anotherDate);
        ...
        const newList = prevList.map(item => {
          if (item._id === modalUser._id) {
              return {
                  ...
                  updateDate: updatedEntryFromServer.updateDate || new Date().toISOString(),
              };
          }
          return item;
  ...
  async function performDeleteOperation(entry) {
    ...
    try {
      ...
      setEntryList(prevList => {
          const newList = prevList.map(e => e._id === entry._id ? updatedEntry : e);
          setFilteredEntryList(newList);
          return newList;
      });
        ...
        <div className="row my-2 mx-auto p-1">
          <div className="col-1">
            <input
              type="checkbox"
              checked={
                selectedIds.length > 0 && selectedIds.length === filteredEntryList.length
              }
              onChange={(ev) => {
